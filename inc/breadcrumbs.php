...
class Breadcrumb_Trail
{
  ...
  protected function set_labels()
  {
    ...
    $defaults            = array(
      'browse'                     => esc_html__('Browse:', 'roach'),
      'aria_label'                     => esc_attr_x('Breadcrumbs', 'breadcrumbs aria label', 'roach'),
      'home'                     => esc_html__('Home', 'roach'),
      'error_404'                     => esc_html__('404 Not Found', 'roach'),
      'archives'                     => esc_html__('Archives', 'roach'),
      'search'                     => esc_html__('Search results for: %s', 'roach'),
      'paged'                     => esc_html__('Page %s', 'roach'),
      'paged_comments'                     => esc_html__('Comment Page %s', 'roach'),
      'archive_minute'                     => esc_html__('Minute %s', 'roach'),
      'archive_week'                     => esc_html__('Week %s', 'roach'),
  ...
  protected function add_singular_items()
  {
    ...
    if (!get_theme_mod('roach_hide_breadcrumb_cats')) {
  ...
  {
    $this->add_rewrite_front_items();
    if (true === $this->args['show_title']) $this->items[] = sprintf($this->labels['archive_minute_hour'], get_the_time(esc_html_x('g:i a', 'minute and hour archives time format', 'roach')));
  }
  protected function add_minute_archive_items()
  {
    $this->add_rewrite_front_items();
    if (true === $this->args['show_title']) $this->items[] = sprintf($this->labels['archive_minute'], get_the_time(esc_html_x('i', 'minute archives time format', 'roach')));
  }
  protected function add_hour_archive_items()
  {
    $this->add_rewrite_front_items();
    if (true === $this->args['show_title']) $this->items[] = sprintf($this->labels['archive_hour'], get_the_time(esc_html_x('g a', 'hour archives time format', 'roach')));
  }
  protected function add_day_archive_items()
  {
    $this->add_rewrite_front_items();
    $year  = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'roach')));
    $month = sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'roach')));
    $day   = sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'roach')));
  ...
  protected function add_week_archive_items()
  {
    $this->add_rewrite_front_items();
    $year = sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'roach')));
    $week = sprintf($this->labels['archive_week'], get_the_time(esc_html_x('W', 'weekly archives date format', 'roach')));
    $this->items[]      = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y'))), $year);
  ...
  protected function map_rewrite_tags($post_id, $path)
  {
    ...
    if (is_array($matches)) {
      foreach ($matches as $match) {
        $tag         = trim($match, '/');
        if ('%year%' == $tag) $this->items[]             = sprintf('<a href="%s">%s</a>', esc_url(get_year_link(get_the_time('Y', $post_id))), sprintf($this->labels['archive_year'], get_the_time(esc_html_x('Y', 'yearly archives date format', 'roach'))));
        elseif ('%monthnum%' == $tag) $this->items[]             = sprintf('<a href="%s">%s</a>', esc_url(get_month_link(get_the_time('Y', $post_id), get_the_time('m', $post_id))), sprintf($this->labels['archive_month'], get_the_time(esc_html_x('F', 'monthly archives date format', 'roach'))));
        elseif ('%day%' == $tag) $this->items[]             = sprintf('<a href="%s">%s</a>', esc_url(get_day_link(get_the_time('Y', $post_id), get_the_time('m', $post_id), get_the_time('d', $post_id))), sprintf($this->labels['archive_day'], get_the_time(esc_html_x('j', 'daily archives date format', 'roach'))));
